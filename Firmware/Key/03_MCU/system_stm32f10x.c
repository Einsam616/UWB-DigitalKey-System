...
//系统时钟（SYSCLK）源选择宏定义
#define  HSI_PLL_SYSCLK    //使用HSI(8MHz)/2作为PLL输入时钟，PLL输出作为SYSCLK时钟源
//#define  HSE_PLL_SYSCLK    //使用HSE(8MHz)作为PLL输入时钟，PLL输出作为SYSCLK时钟源
...
static void SetSysClockTo72(void)
{
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    /* AHB总线时钟频率 HCLK = SYSCLK */
    RCC->CFGR |= (uint32_t)RCC_CFGR_HPRE_DIV1;
      
    /* 高速APB时钟频率 PCLK2 = HCLK */
    RCC->CFGR |= (uint32_t)RCC_CFGR_PPRE2_DIV1;
    
    /* 低速APB时钟频率 PCLK1 = HCLK/2 */
    RCC->CFGR |= (uint32_t)RCC_CFGR_PPRE1_DIV2;
...
static void SetSysClockTo64(void)
{
...
#ifdef HSI_PLL_SYSCLK	  //使用HSI/2作为PLL输入时钟，PLL输出作为SYSCLK
  /* Enable HSI */    
...
#ifdef HSE_PLL_SYSCLK	    //使用HSE作为PLL输入时钟，PLL输出作为SYSCLK
  /* Enable HSE */    
  RCC->CR |= ((uint32_t)RCC_CR_HSEON);
  ...
  if (HSIStatus == (uint32_t)0x01 || HSEStatus == (uint32_t)0x01)
  {
    /* Enable Prefetch Buffer */
    FLASH->ACR |= FLASH_ACR_PRFTBE;
