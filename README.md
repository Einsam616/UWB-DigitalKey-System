...
- Lock 端：接收并校验 CC1101 业务帧；读取 PA6/PA7/PB9/PB10 四位拨码作为验证 ID；按距离和前方 `±45°` 区域切换等待、感应、迎宾、开锁、闭锁状态；驱动彩屏、D1～D5 指示灯、蜂鸣器和 USART1 调试输出。
...
- Key 当前固定发送 ID `1101`（代码中的 `APP_KEY_ID=0x0D`）；Lock 的 PA6/PA7/PB9/PB10 设置待验证 ID，ON 接地表示 `1`，从左到右对应 bit3～bit0。
- Lock 拨成 `1101` 时身份通过；任意一位不一致时保留测距显示，但保持闭锁。Lock 运行中检测到拨码变化后立即重新判定当前钥匙。
- 角度超出前方 `±45°` 时判为区域外。
- 相对圆柱边界距离：`0～1 m` 开锁区，`1～2 m` 迎宾区，`2～3 m` 感应区，超过 `3 m` 闭锁。
- 各边界使用 `50 mm` 回差；连续超过 `1 s` 没有有效帧时自动闭锁并清除位置显示。
- Lock 指示灯：D1 运行、D2 无线链路、D3 迎宾区、D4 闭锁、D5 开锁。
...
PA13/PA14 保留 SWD 下载调试，PB2 保持 BOOT1 下拉。Key 与 Lock 的 CC1101 只有本地 CSN 不同：Key 使用 PB11，Lock 使用 PB12。
